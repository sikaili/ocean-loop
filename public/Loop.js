...
var Loop = function Loop(r, _x, _y) {
  ...
  _defineProperty(this, "update", function () {
    _this.clock1 < 170 || _this.coli.length > 10 || millis() - _this.start > 10000 ? _this.pos.add(_this.go) : "";
  });
  ...
  _defineProperty(this, "display", function (array) {
    ...
    for (var i = 0; i < PI * 2; i += 0.07) {
      ...
      strokeWeight(2 / 1000 * (width + height) / 2 * _this.clock1 / 255 * pixelDensity() ^ 2 * map(_this.clock1, 140, 240, 0.7, 1.3));
      point(x + i, y + i); // this.coli.mouse && this.r < 30 ? vertex(x * noise(i), y) : "";
    }
  ...
  _defineProperty(this, "inter", function (array) {
    for (var i = 0; i < array.length; i++) {
      if (_this != array[i]) {
        ...
        if (distance < _this.r + array[i].r && _this.coli.indexOf(array[i]) == -1 && distance > 2) {
...
function spinningPlate(r, i, clock1, clock, length) {
  var n = Math.floor(clock1 + length / 2) % 5;
