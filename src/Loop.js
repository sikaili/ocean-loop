class Loop {
  ...
  update = () => {
    this.clock1 < 170 || this.coli.length > 10 || millis() - this.start > 7000 ? this.pos.add(this.go) : "";
    // this.coli.mouse && this.r - this.rInit < 100 ? this.r += 0.1 : this.r -= this.r > this.rInit ? 0.1 : 0;
  }
  display = (array, amp) => {
    push();
    fill(0, 0);
    let color = [...(this.coli.length % 3 === 1 && this.clock1 > 160) || this.coli.mouse ? [random(0, 255), random(0, 255), random(0, 255), (this.clock1 / 1.5 + 30 + amp * 150)] : [255, this.clock1 / 1.8 + 30]]
    stroke(color);
    beginShape();
